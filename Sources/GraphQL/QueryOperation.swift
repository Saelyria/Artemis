...
//class QueryObject<O: AnyAggregateOperation & Querying> {
//    private let operations: O
//    private let name: String?
//
//    init(name: String? = nil, @QueryOperationBuilder _ operationsBuilder: () -> O) {
//        self.name = name
...
class QueryOperation<T: Queryable>: AnyQueryOperation, AnyAggregateOperation, GraphQLItem, Querying {
...
protocol AnyQueryOperation: Querying {
...
extension AnyQueryOperation {
    ...
    init<Q: GraphQLItem & Querying>(alias: String? = nil, @QuerySubSelectionBuilder query: () -> Q) where Q.T == T {
        let items = query()
        self.init(alias: alias, renderedSubQuery: items.render(), aggregateItems: items, arguments: [])
    }
    
    init<Q: GraphQLItem & Querying>(alias: String? = nil, arguments: T.Args..., @QuerySubSelectionBuilder query: () -> Q) where Q.T == T {
        let items = query()
