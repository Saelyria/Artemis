...
extension TestInterface3_Int_TypeTests {
    ...
    func testArrayAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i3_ints}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res?.count, 2)
        XCTAssertEqual(res?[safe: 0], 123)
        XCTAssertEqual(res?[safe: 1], 321)
    ...
    func testArrayArgsAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i3_intsArgs\(testArgs)}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res?.count, 2)
        XCTAssertEqual(res?[safe: 0], 123)
        XCTAssertEqual(res?[safe: 1], 321)
