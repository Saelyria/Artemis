...
extension TestInterface4_TestObject_TypeTests {
    ...
    func testArrayAliasOnObjectParse() throws {
        ...
        XCTAssertEqual(query.render(), "{testObject{alias:i4_testObjects{int}}}")
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
    ...
    func testArrayArgsAliasOnObjectParse() throws {
        ...
        XCTAssertEqual(query.render(), "{testObject{alias:i4_testObjectsArgs\(testArgs){int}}}")
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
...
extension TestInterface4_TestObject_TypeTests {
    ...
    func testArrayAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i4_testObjects{int}}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res?.count, 2)
        XCTAssertEqual(res?[safe: 0]?.int, 321)
        XCTAssertEqual(res?[safe: 1]?.int, 123)
    ...
    func testArrayArgsAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i4_testObjectsArgs\(testArgs){int}}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res?.count, 2)
        XCTAssertEqual(res?[safe: 0]?.int, 321)
        XCTAssertEqual(res?[safe: 1]?.int, 123)
