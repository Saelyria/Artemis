...
extension TestInterface4_Float_TypeTests {
...
extension TestInterface4_Float_TypeTests {
    func testSingleOnObjectRender() {
        ...
        XCTAssertEqual(res?.i4_float, 1.23)
    }
    ...
    func testSingleArgsOnObjectRender() {
        ...
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
        XCTAssertEqual(res?.i4_floatArgs, 1.23)
    }
    ...
    func testArrayOnObjectRender() {
        ...
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
        XCTAssertEqual(res?.i4_floats?.count, 2)
        XCTAssertEqual(res?.i4_floats?[safe: 0], 1.23)
        XCTAssertEqual(res?.i4_floats?[safe: 1], 3.21)
    }
    ...
    func testArrayArgsOnObjectRender() {
        ...
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
        XCTAssertEqual(res?.i4_floatsArgs?.count, 2)
        XCTAssertEqual(res?.i4_floatsArgs?[safe: 0], 1.23)
        XCTAssertEqual(res?.i4_floatsArgs?[safe: 1], 3.21)
    }
}
...
extension TestInterface4_Float_TypeTests {
    func testSingleAliasOnObject() throws {
        ...
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
        let aliased = res?.get(\.i4_float, alias: "alias")
    ...
    func testSingleArgsAliasOnObject() throws {
        ...
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
        let aliased = res?.get(\.i4_floatArgs, alias: "alias")
        XCTAssertEqual(aliased, 1.23)
    }
    ...
    func testArrayArgsAliasOnObjectParse() throws {
        ...
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
        let aliased = res?.get(\.i4_floatsArgs, alias: "alias")
        XCTAssertEqual(aliased?.count, 2)
        XCTAssertEqual(aliased?[safe: 0], 1.23)
        XCTAssertEqual(aliased?[safe: 1], 3.21)
        XCTAssertNil(res?.i4_floatsArgs)
    }
}
...
extension TestInterface4_Float_TypeTests {
    func testSingleAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i4_float}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res, 1.23)
    }
    ...
    func testSingleArgsAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i4_floatArgs\(testArgs)}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res, 1.23)
    }
    ...
    func testArrayAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i4_floats}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res?.count, 2)
        XCTAssertEqual(res?[safe: 0], 1.23)
        XCTAssertEqual(res?[safe: 1], 3.21)
    ...
    func testArrayArgsAliasOnFragment() {
        ...
        XCTAssertEqual(query.render(), "{...fragName},fragment fragName on Query{alias:i4_floatsArgs\(testArgs)}")
        let res = try? query.createResult(from: response)
        XCTAssertEqual(res?.count, 2)
        XCTAssertEqual(res?[safe: 0], 1.23)
        XCTAssertEqual(res?[safe: 1], 3.21)
...
// MARK: - Tests to ensure fragments on TestInterface4 can be used on selections of Float or [Float]
...
extension TestInterface4_Float_TypeTests {
    func testSingleOnTestInterface4Fragment() {
        ...
        XCTAssertEqual(query.render(), "{testObject{...fragName}},fragment fragName on TestInterface4{i4_float}")
        let res: Partial<TestObject>? = try? query.createResult(from: response)
        XCTAssertEqual(res?.values.count, 1)
        XCTAssertEqual(res?.i4_float, 1.23)
    }
    ...
    func testArrayOnObjectFragmentRender() {
        ...
        XCTAssertEqual(query.render(), "{testObjects{...fragName}},fragment fragName on TestInterface4{i4_float}")
        let res: [Partial<TestObject>]? = try? query.createResult(from: response)
        XCTAssertEqual(res?.count, 2)
        XCTAssertEqual(res?[safe: 0]?.i4_float, 1.23)
        XCTAssertEqual(res?[safe: 1]?.i4_float, 3.21)
    }
}
